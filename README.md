...
Planner schema v3 keeps the application framework and user-owned content separate. The repository contains no personal roadmap seed, category names, training plans, or performance target values. Those values live only in `planner-data.json` under `categories`, `settings`, `fitness.plans`, `performance.targets`, and the normal record collections. Use **Personal Planner → Data settings** to create, rename, rebind, or delete categories, configure which generic module a category opens, and manage training plans and performance activity targets. Existing task, event, roadmap, fitness, and performance screens remain the CRUD entry points for their records. Writes are atomic and the store retains `planner-data.json.bak`; schema-v2 upgrades should also retain a one-time `planner-data.json.pre-v3.bak` before moving prior embedded preferences into local data.
